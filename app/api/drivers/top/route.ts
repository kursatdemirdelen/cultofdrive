...
export async function GET() {
  try {
    const { data, error } = await supabase
      ...
      .select(`
        user_id,
        user_profiles!cars_user_id_fkey(display_name, slug, avatar_url)
      `)
      .not("user_id", "is", null);

    if (error) throw error;

    const driverCounts = (data || []).reduce((acc: Record<string, { display_name: string; slug: string; avatar_url?: string; count: number }>, car: any) => {
      ...
      if (!acc[key]) {
        acc[key] = { 
          ...
          slug: profile.slug || profile.display_name,
    ...
    const drivers = Object.values(driverCounts)
      .map(({ display_name, slug, avatar_url, count }) => ({ 
        owner: display_name, 
        slug, 
        avatar_url,
        car_count: count 
